...
{
    public sealed class AllySkillExecutor : ISkillUseResultExecutor
    {
        private readonly BattleAlly _battleAlly;
        private readonly EntityStat _allyStat;
        ...
        public AllySkillExecutor(BattleAlly battleAlly, StatSO procChanceStat, BattleSkillUseService skillUseService)
        {
            _battleAlly = battleAlly;
            _procChanceStat = procChanceStat;
            _allyStat = battleAlly != null ? battleAlly.GetModule<EntityStat>() : null;
        ...
        public int PickRandomUsableSkillIndex(Func<int, bool> canUse)
        {
            return _selector.PickRandomFiltered(skillData => !IsDefenseSkill(skillData), canUse);
        }
        
        public int PickBestDefenseSkillIndex()
        {
            return _selector.PickHighestDamage(IsDefenseSkill);
        }
            ...
            SkillTargetCandidates candidates, bool canActivateChain)
        {
            ...
            if (_battleAlly == null || _battleAlly.IsDead || skillData == null || !_skillUseGateway.IsAvailable)
            {
            ...
            try
            {
                SkillUseResult result = await _skillUseGateway.UseAsync(_battleAlly, skillData, 
            ...
            {
                Debug.LogException(exception, _battleAlly);
                LastUseResult = BtSkillUseResult.Failed;
            }
            ...
            SkillTargetCandidates candidates)
        {
            ...
            {
                TargetType.Direct => ContainsCandidate(candidates, directTarget) &&
                                     CanBeActiveDirectTarget(directTarget, true),
